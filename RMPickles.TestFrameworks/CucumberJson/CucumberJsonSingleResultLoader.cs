...
namespace RMPickles.Core.TestFrameworks.CucumberJson
{
    public class CucumberJsonSingleResultLoader : ISingleResultLoader
    {
        public SingleTestRunBase Load(FileInfo fileInfo)
        {
            return new CucumberJsonSingleResults(this.ReadResultsFile(fileInfo));
        }

        private List<Feature> ReadResultsFile(FileInfo testResultsFile)
        {
            List<Feature> result;
